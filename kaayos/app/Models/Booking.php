...
class Booking extends Model
{
    ...
    public function decline(?string $reason = null, ?int $userId = null): void
    {
        if (! in_array($this->status, [self::STATUS_NEW])) {
            throw new \InvalidArgumentException(
                "Cannot decline a booking that is '{$this->status}'. Only 'new' bookings can be declined."
            );
        }
        ...
        DB::transaction(function () use ($reason, $userId) {
            ...
            $fresh->update([
                ...
                'declined_at' => now(),
                'decline_reason' => $reason ?? 'Declined',
            ]);

            $fresh->history()->create([
                'user_id' => $userId,
                'old_status' => $oldStatus,
                'new_status' => self::STATUS_DECLINED,
                'notes' => $reason,
            ]);

            $this->status = $fresh->status;
            $this->declined_at = $fresh->declined_at;
            $this->decline_reason = $fresh->decline_reason;
            $this->syncOriginalAttribute('status');
        });
    }
